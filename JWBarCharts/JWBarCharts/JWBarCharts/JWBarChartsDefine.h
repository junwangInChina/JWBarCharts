//
//  JWBarChartsDefine.h
//  JWBarCharts
//
//  Created by wangjun on 2018/11/15.
//  Copyright © 2018年 wangjun. All rights reserved.
//

#define JW_BARCHARTS_SCREEN_WIDTH       ([UIScreen mainScreen].bounds.size.width)
#define JW_BARCHARTS_SCREEN_HEIGHT      ([UIScreen mainScreen].bounds.size.height)
#define JW_BARCHARTS_SCREEN_SCALE       ([UIScreen mainScreen].scale)
#define JW_BARCHARTS_YAXIS_WIDTH        30
#define JW_BARCHARTS_YAXIS_OUT          20
#define JW_BARCHARTS_X_SEPARTOR_NUM     4

#define JW_BC_WS(weakSelf)  __weak __typeof(&*self)weakSelf = self;

